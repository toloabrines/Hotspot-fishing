...
export const createCheckoutSession = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }): Promise<CheckoutSessionResult> => {
    try {
      ...
      const { ensureInclusivePrice } = await import("@/lib/stripe-price.server");
      const stripePrice = await ensureInclusivePrice(stripe, data.priceId);
      ...
      // Paquetes de consultas extra de IA: pago único con créditos asociados.
      const { AI_PACK_BY_PRICE_ID } = await import("@/lib/ai-packs");
      const pack = AI_PACK_BY_PRICE_ID[data.priceId];
...
export const createPortalSession = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }): Promise<PortalSessionResult> => {
    ...
    if (!sub?.stripe_customer_id) {
      return { error: "Todavía no tienes ninguna suscripción activa." };
    }
